...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        //System.out.print("ProgramNode nameAnalysis\n");
        SymTable symTab = new SymTable();
        myDeclList.nameAnalysis(symTab);
        Sym s;
        if ((s = symTab.lookupGlobal("main")) == null || !(s.getType()
                .isFnType()))
    ...
    // go down one level to generate code for declist
    public void codeGen() {
...
class DeclListNode extends ASTnode {
    ...
    // returns the size of decl list for offset calculation
    public int size() {
        return myDecls.size();
    }
    ...
    public void nameAnalysis(SymTable symTab, SymTable globalTab) {
        //System.out.print("DeclListNode name Analysis\n");
    ...
    public void unparse(PrintWriter p, int indent) {
        ...
        try {
            ...
            System.err.println("unexpected NoSuchElementException in " +
                    "DeclListNode.print");
            System.exit(-1);
        }
    }
    ...
    public void codeGen() {
        // Iterate through all declNode to generate code
        for (DeclNode node : myDecls) {
            node.codeGen();
        }
...
class FormalsListNode extends ASTnode {
    ...
    public List<Type> nameAnalysis(SymTable symTab) {
        ...
        for (FormalDeclNode node : myFormals) {
            ...
            if (sym != null) {
                // save the offset for formals
                //System.out.print("sym is null in formallist name analysis");
                sym.offset = offset;
                offset += 4;
...
class FnBodyNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        //System.out.print("Function boday name analysis\n");
        myDeclList.nameAnalysis(symTab);
        //System.out.print("Function boday name analysis going into stmt
        // list\n");
...
class StmtListNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int offset) {
        // System.out.print("Going into StmtListNode name analysis\n");
    ...
    public void typeCheck(List<Type> typeList) {
        ...
        try {
            for (ExpNode node : myExps) {
                ...
                if (!actualType.isErrorType()) {        // if this is not an
                    // error
...
class VarDeclNode extends DeclNode {
    ...
    public Sym nameAnalysis(SymTable symTab, SymTable globalTab) {
        ...
        if (!badDecl) {  // insert into symbol table
            try {
                ...
                // if there is only one hashtable in the table then this
                // variable is global
                if (symTab.size() == 1) {
                    sym.isGloabl_set();
                }
    ...
    @Override
    public void codeGen() {
        // If it is global generate .data .align _v: .space 4
        ...
        // If it is not global it would be generated in the funcdecl because
...
class FnDeclNode extends DeclNode {
     ...
     */
    public Sym nameAnalysis(SymTable symTab) {
        // System.out.print("FnDeclNode Analysis\n");
        String name = myId.name();
        FnSym sym = null;
        ...
        if (sym != null) {
            ...
            // Set the offset for parameters
            this.parameterOffset = 4 * this.myFormalsList.length();
            sym.offset = this.parameterOffset;
        }

        int interOffset = 8 + this.parameterOffset;

        myBody.nameAnalysis(symTab, interOffset +  myBody
                .getDeclListSize());
        //fnbody
        for (DeclNode node : myBody.getDecList()) {
            if (node instanceof VarDeclNode) {
                ((VarDeclNode) node).getId().sym().offset = interOffset;
                // increment the offset by 4
                interOffset += 4;
            }
        }
        // the locals should start from the top of all p
        localOffset = interOffset - 8 - parameterOffset;
    ...
    public void codeGen() {
        ...
        // function entry
        Codegen.generateIndexed("sw", Codegen.RA, Codegen.SP, 0, "push return" +
                " addr");
        ...
        Codegen.generate("#set the FP");
        Codegen.generate("addu", Codegen.FP, Codegen.SP,
                parameterOffset + 8 +"");

        Codegen.generate("subu", Codegen.SP, Codegen.SP, localOffset + "");
        Codegen.generate("#push space for locals");

        // Function Body
        String label = "_" + myId.name() + "_Exit";
        this.myBody.codeGen(label);

        // Function Exit

        // label for exit return
        Codegen.genLabel(label);

        Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, -(this
                .parameterOffset), "load return address");
        Codegen.generateWithComment("move", "save control link", Codegen.T0,
                Codegen.FP);
        Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP, -(this
                .parameterOffset + 4), "restore FP");
        Codegen.generateWithComment("move", "restore SP", Codegen.SP, Codegen
                .T0);
        ...
        if (myId.name().equals("main")) {
            ...
            Codegen.generateWithComment("jr", "Return jump", Codegen.RA);
        }
...
abstract class StmtNode extends ASTnode {
    ...
    // default nameAnalysis with offset
    public void nameAnalysis(SymTable symTable, int offset) {
        nameAnalysis(symTable);
    }
...
class AssignStmtNode extends StmtNode {
    ...
    public void codeGen(String label) {
        ...
        // pop the assigned value into T0;
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen(String label1) {
        myExp.codeGen();
...
class WriteStmtNode extends StmtNode {
    ...
    public void codeGen(String label) {
        ...
        //delete after
        Codegen.generate("#write");

        Codegen.genPop(Codegen.A0);
        Codegen.generate("li", Codegen.V0, (myExp instanceof StringLitNode)
                ? 4 : 1);
        Codegen.generate("syscall");
...
class IfStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        ...
        symTab.addScope();
        myDeclList.nameAnalysis(symTab);
        myStmtList.nameAnalysis(symTab, 4 * myDeclList.getMyDecls().size() +
                offset);
        //TODO
        this.offset = 0;
        for (DeclNode decl : myDeclList.getMyDecls()) {
            if (decl instanceof VarDeclNode) {
                ((VarDeclNode) decl).getId().sym().offset = offset + this
                        .offset;
                this.offset += 4;
            }
        }
    ...
    public void codeGen(String label) {
        ...
        Codegen.generate("bne", Codegen.T0, "1", l1);
        Codegen.generate("subu", Codegen.SP, Codegen.SP, offset);
        myStmtList.codeGen(label);
        Codegen.generate("addu", Codegen.SP, Codegen.SP, offset);
        Codegen.genLabel(l1);
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        ...
        //TODO
        this.offset = 0;
        for (DeclNode decl : myThenDeclList.getMyDecls()) {
            if (decl instanceof VarDeclNode) {
                ((VarDeclNode) decl).getId().sym().offset = offset + this
                        .offset;
                this.offset += 4;
        ...
        myThenStmtList.nameAnalysis(symTab, 4 * myThenDeclList.getMyDecls()
                .size() + offset);
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
        ...
        myElseStmtList.nameAnalysis(symTab, offset + 4 * myElseDeclList
                .getMyDecls().size());
        //TODO
        //else block to add offset to each sym of decl list
        this.offsetElse = 0;
        for (DeclNode decl : myElseDeclList.getMyDecls()) {
            if (decl instanceof VarDeclNode) {
                ((VarDeclNode) decl).getId().sym().offset = offset + this
                        .offsetElse;
                this.offsetElse += 4;
    ...
    public void codeGen(String label) {
        ...
        Codegen.genLabel(l1);
        Codegen.generate("subu", Codegen.SP, Codegen.SP, offsetElse);
        myElseStmtList.codeGen(label);
        Codegen.generate("addu", Codegen.SP, Codegen.SP, offsetElse);
        Codegen.genLabel(l2);
    }
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen(String label) {
        ...
        myExp.codeGen();
        Codegen.genPop(Codegen.T0);
        Codegen.generate("bne", Codegen.T0, "1", l2);
        Codegen.generate("subu", Codegen.SP, Codegen.SP, offset);
        myStmtList.codeGen(label);
        Codegen.generate("addu", Codegen.SP, Codegen.SP, offset);
...
class StringLitNode extends ExpNode {
    ...
    public void codeGen() {
        String label = dict.get(myStrVal);
        if (label == null) {
            String strLabel = Codegen.nextLabel();
            dict.put(myStrVal, strLabel);
            Codegen.generate(".data");
            ...
            // push the result onto the block
            Codegen.genPush(Codegen.T0);
        }
    }
...
class IdNode extends ExpNode {
    ...
    public void genJumpAndLink() {
        Codegen.generate("jal", (myStrVal.equals("main")) ? myStrVal :
                "_" + myStrVal);
    ...
    public void genAddr() {
        if (mySym.isGlobal) {
            ...
            Codegen.generateIndexed("la", Codegen.T0, Codegen.FP, -mySym
                    .offset);
            Codegen.genPush(Codegen.T0);
        }
    }
    ...
    public void codeGen() {
        if (mySym.isGlobal) {
            ...
            Codegen.generateIndexed("lw", Codegen.T0, Codegen.FP, -mySym
                    .offset);
            Codegen.genPush(Codegen.T0);
        }
    }
            ...
            if (loc.badAccess) {  // if errors in processing myLoc
                ...
                if (sym == null) {  // no struct in which to look up RHS
                    ErrMsg.fatal(loc.lineNum(), loc.charNum(),
                            "Dot-access of non-struct type");
                    badAccess = true;
                } else {  // get the struct's symbol table in which to lookup
                    // RHS
                    if (sym instanceof StructDefSym) {
                        structSymTab = ((StructDefSym) sym).getSymTable();
                    } else {
                        System.err.println("Unexpected Sym type in " +
                                "DotAccessExpNode");
                        System.exit(-1);
                    }
                }
...
class AssignNode extends ExpNode {
     ...
     */
    public void nameAnalysis(SymTable symTab) {
        // System.out.print("Assign Node Name Analysis\n");
        myLhs.nameAnalysis(symTab);
        //System.out.print("Assign Node going into the expression list\n");
        myExp.nameAnalysis(symTab);
    }

    public void codeGen() {
        //LHS ID RHS ID, LHS ID RHS EXP, TEST
        if (myLhs instanceof IdNode) {
            ...
            //((DotAccessExpNode)(myLhs)).myId.genAddr();
        }
    ...
    public Type typeCheck() {
        ...
        if (!typeLhs.equals(typeExp) && !typeLhs.isErrorType() && !typeExp
                .isErrorType()) {
...
class UnaryMinusNode extends UnaryExpNode {
    ...
    public void codeGen() {
        ...
        // Negate number by subtract the number from 0
...
class NotNode extends UnaryExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.genPop(Codegen.T0);
        Codegen.generate("li", Codegen.T1, "0");
        Codegen.generate("nor", Codegen.T0, Codegen.T0, Codegen.T1);
        Codegen.genPush(Codegen.T0);
    }
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        myExp1.codeGen();
        Codegen.genPop(Codegen.T0);
        Codegen.generate("bne", Codegen.T0, "0", l1);
        Codegen.genPush(Codegen.T0);
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        ...
        if (myExp1 instanceof StringLitNode) {
            String val1 = ((StringLitNode) myExp1).myStrVal;
            String val2 = ((StringLitNode) myExp2).myStrVal;
            if (val1.equals(val2)) {
                bool = 1;
            }
            Codegen.generate("li", Codegen.T0, bool);
        } else {
            String trueCase = Codegen.nextLabel();
            String exitCase = Codegen.nextLabel();

            // evaluate value from experssions
            myExp1.codeGen();
            myExp2.codeGen();

            // Get the result from left right
            Codegen.genPop(Codegen.T1);
            Codegen.genPop(Codegen.T0);

            // Compare
            Codegen.generate("beq", Codegen.T0, Codegen.T1, trueCase);

            // false case
            Codegen.generate("li", Codegen.T0, 0);
            Codegen.generate("b", exitCase);

            // true case
            Codegen.genLabel(trueCase);
            Codegen.generate("li", Codegen.T0, 1);

            Codegen.genLabel(exitCase);
        }
...
class NotEqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        ...
        if (myExp1 instanceof StringLitNode) {
            ...
            // Compare
            Codegen.generate("bne", Codegen.T0, Codegen.T1, trueCase);
...
class LessNode extends RelationalExpNode {
    ...
    public void codeGen() {
        String falseCase = Codegen.nextLabel();
        String exitCase = Codegen.nextLabel();

        // Get the results from the left and right side
        this.myExp1.codeGen();
        this.myExp2.codeGen();
        ...
        // Codegen generate
        Codegen.generate("sub", Codegen.T0, Codegen.T0, Codegen.T1);
        Codegen.generate("bgez", Codegen.T0, falseCase);

        // False case
        Codegen.generate("li", Codegen.T0, 1);
        Codegen.generate("b", exitCase);

        Codegen.genLabel(falseCase);
        // Exit case
        Codegen.generate("li", Codegen.T0, 0);
...
class GreaterNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // Codegen generate
        Codegen.generate("sub", Codegen.T0, Codegen.T0, Codegen.T1);
        Codegen.generate("blez", Codegen.T0, falseCase);
...
class LessEqNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // Codegen generate
        Codegen.generate("sub", Codegen.T0, Codegen.T0, Codegen.T1);
        Codegen.generate("blez", Codegen.T0, trueCase);

        //
        Codegen.generate("li", Codegen.T0, 0);
        Codegen.generate("b", exitCase);

        Codegen.genLabel(trueCase, "LABEL FOR TRUE");
...
class GreaterEqNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // Codegen generate
        Codegen.generate("sub", Codegen.T0, Codegen.T0, Codegen.T1);
        Codegen.generate("bgtz", Codegen.T0, trueCase);
